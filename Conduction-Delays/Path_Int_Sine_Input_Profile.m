%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 %%%% external input to initiate bubble, no idiothetic cue, w sym.
   ...
   w=ws-w_inh;
   I_ext=zeros(nn,1); for i=-10:10; I_ext(nn/2+i)=50; end
...
dt=.08; %%% 'infintesimal' time step
tau_m=1;
t_span=t3; %%% number of time steps
%I_ext_time=1000; %%% time duration for which to apply I_ext
delay_r_mu=100; %%% ditto from above except now...
delay_r_sig=20; %%% ditto ...
...
for a=t1+1:t2;
    for b=1:nn;
        I_ext(b,a)=I_ext_max*(abs(amp*sin(2*pi*(a-t1)/(t2-t1))+nn/2-b)<thickness);
    end
end
...
%             u_out_j(k)=u_output(k,i-r);
        ...
        u_output(j,i)=u_output(j,i-1)*(1-dt*tau_m)+dt*tau_m*(w_sum*dx+I_ext(j)*0);
    end
end
%%t2:
...
active_head_n=u_output>-130; %%% Gauge the lower limit for active_head_n
...
plot(1:t_span,head_dir,1:t_span,input_dir,[0 0],[30 70])
